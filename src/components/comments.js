...
const CommentForm = ({ appId, pageId, parentId = null, replyToName = null, onSubmitted, onCancel }) => {
...
const CommentItem = ({ comment, appId, pageId, refresh }) => {
  ...
  const displayDate = comment.createdAt ? new Date(comment.createdAt).toLocaleString(undefined, {
    year: 'numeric',
    month: 'numeric',
    day: 'numeric',
    hour: '2-digit',
    ...
    <div style={{ marginBottom: '1.5rem' }}>
      <div style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
          ...
          <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>{displayDate}</span>
        </div>
        <div 
        ...
        {replying && (
          <CommentForm 
            ...
            onSubmitted={() => { setReplying(false); refresh(); }}
      ...
      {comment.replies && comment.replies.length > 0 && (
        <div style={{ marginLeft: '1.5rem', marginTop: '1rem' }}>
          {comment.replies
            ...
            .map(reply => (
              <CommentItem key={reply.id} comment={reply} appId={appId} pageId={pageId} refresh={refresh} />
            ))}
        </div>
      )}
    </div>
...
const Comments = ({ appId, pageId }) => {
  ...
  const fetchComments = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${CUSDIS_API_HOST}/api/open/comments?appId=${encodeURIComponent(appId)}&pageId=${encodeURIComponent(pageId)}&limit=100`,
        { cache: 'no-store' }
      )
      ...
      const processComment = (c) => {
        ...
        const processed = { ...rest, replies: [] }
        
        if (replies && replies.data && Array.isArray(replies.data)) {
          processed.replies = replies.data.map(processComment)
            .sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)) // Sort replies oldest first
        }
  ...
  if (!appId) return (
    <div style={{ padding: '1rem', border: '1px dashed var(--color-accent)', borderRadius: '4px' }}>
      <p style={{ margin: 0 }}>Please configure your Cusdis App ID in <code>src/templates/blog-post.js</code></p>
    </div>
  )
    ...
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        {loading ? (
          ...
          comments.map(c => (
            <CommentItem key={c.id} comment={c} appId={appId} pageId={pageId} refresh={fetchComments} />
          ))
        )}
      </div>

      <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid var(--color-accent)' }}>
        <h4 style={{ marginTop: 0, marginBottom: '1rem' }}>Leave a comment</h4>
        <CommentForm appId={appId} pageId={pageId} onSubmitted={fetchComments} />
      </div>
    </div>
  )
}
